...
For better DX, you can define your state structure directly in the class body. This provides IDE autocompletion and automatic validation.
...
class MyAgent(Agent):
    state = UserState  # Blueprint

    async def update_profile(self, name: str):
        # Full autocomplete for .name!
        await self.update_state(name=name)
```

You can also use a plain dictionary as a blueprint:
...
See the [Deployment Guide](/how-to/deployment) for details.

## API Reference

<CardGroup cols={2}>
  <Card title="Agent API" icon="book" href="/reference/agent">
    Full methods and properties
  </Card>
  <Card
    title="Storage Backends"
    icon="database"
    href="/reference/agent-backends"
  >
    MemoryStorage, SQLiteStorage, and custom
  </Card>
  <Card title="Agent Server" icon="server" href="/explanation/agent-server">
    Hosting agents
  </Card>
  <Card title="Deployment" icon="rocket" href="/how-to/deployment">
