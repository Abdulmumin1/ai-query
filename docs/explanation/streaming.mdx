...
`stream_text` can also surface provider thinking output separately through `on_reasoning_event`, without mixing it into the visible assistant text stream.
...
<Note>
`stream_text` returns immediately with a [`TextStreamResult`](/reference/types/results) object. The actual streaming happens when you iterate over `text_stream`.
</Note>
...
If a provider exposes visible thinking output, receive it with `on_reasoning_event`:
...
See [`Thinking`](/explanation/thinking) for provider differences and guidance.
...
<CardGroup cols={2}>
  <Card title="Tools" icon="wrench" href="/explanation/tools">
    Add function calling to your streams
  </Card>
  <Card title="Agents" icon="robot" href="/explanation/agents">
