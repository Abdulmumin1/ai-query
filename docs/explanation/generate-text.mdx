...
If you want provider thinking output during streaming, see [`Thinking`](/explanation/thinking). `generate_text()` controls reasoning depth, but thinking output itself is exposed through `stream_text(..., on_reasoning_event=...)`.
