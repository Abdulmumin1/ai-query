...
<ResponseField name="termination" type="TurnTermination | None">
  Structured framework-level reason the loop ended. Inspect this instead of
  inferring step limits, hook stops, or tool termination from empty text.
</ResponseField>
...
<ResponseField name="AbortError" type="Exception">
  Raised when `signal` is aborted. The exception carries `termination` when the
  runtime can attach structured diagnostics.
</ResponseField>
