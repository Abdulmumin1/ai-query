...
Reasoning output emitted separately from assistant text during streaming.
...
Use `stream_text(..., on_reasoning_event=...)` to receive these events. Provider continuity metadata such as signatures or opaque state is exposed through `data` and should not be rendered as assistant text by default.

For the higher-level explanation of reasoning controls vs visible thinking output, see [`Thinking`](/explanation/thinking).
