...
<ResponseField name="text_stream" type="AsyncIterator[str]">
  Async iterator yielding streamed text chunks.
</ResponseField>
...
`usage` is kept as a backward-compatible alias for `cumulative_usage`.
Use `step_usage` for the current step's token delta.
