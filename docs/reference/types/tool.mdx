...
```python
from ai_query import ToolExecutionContext, tool

@tool
async def work(value: str, ctx: ToolExecutionContext) -> str:
    ctx.signal.throw_if_aborted()
    await ctx.emit_progress("working", data={"value": value})
    return value
```

The annotated parameter is framework-owned and omitted from the generated JSON
Schema. Exactly one `ToolExecutionContext` parameter is allowed. It exposes the
tool call identity, step, abort signal, optional agent/turn identity, immutable
metadata, and correlated progress emission.
...
For multimodal results, set `result` to `ToolOutput`:

```python
from ai_query import FilePart, ImagePart, TextPart, ToolOutput

ToolOutput(content=[
    TextPart(text="Inspection complete"),
    ImagePart(image=image_bytes, media_type="image/png"),
    FilePart(data=pdf_bytes, media_type="application/pdf", filename="report.pdf"),
])
```

`ToolOutput.content` accepts only `TextPart`, `ImagePart`, and `FilePart`.
Provider adapters normalize these parts to supported upstream formats and raise
`UnsupportedToolOutputError` when lossless conversion is unavailable.
