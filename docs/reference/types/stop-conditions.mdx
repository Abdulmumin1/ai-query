...
StopCondition = Callable[
    [list[StepResult]],
    bool | StopDecision | Awaitable[bool | StopDecision],
]
```

A stop condition receives all completed steps and returns `True` to stop,
`False` to continue, or a named `StopDecision`.
...
Returning booleans remains fully supported. Return `StopDecision` when product
code needs a stable condition name and reason in `result.termination`.
...
```python
from ai_query import StopDecision, generate_text
from ai_query.providers import google
from ai_query.types import StepResult

def stop_on_error(steps: list[StepResult]) -> StopDecision:
