...
<ParamField path="cached_tokens" type="int">
  Input tokens read from a provider prompt or context cache. Zero when absent or unreported.
</ParamField>

<ParamField path="cache_write_tokens" type="int">
  Input tokens written to a provider cache. Zero when absent or unreported.
</ParamField>

<ParamField path="cache_miss_tokens" type="int">
  Input tokens explicitly reported as cache misses. This is not inferred for providers that do not report it.
</ParamField>
...
print(f"Cached input tokens: {result.usage.cached_tokens}")
...
def estimate_cost(usage: Usage) -> float:
    input_cost = (usage.input_tokens / 1000) * PRICE_PER_1K_INPUT
    output_cost = (usage.output_tokens / 1000) * PRICE_PER_1K_OUTPUT
