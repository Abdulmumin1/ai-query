...
| `tool_execution.progress` | Correlated progress emitted by `ToolExecutionContext`. |
...
`ToolCallReadyEvent`,
`ToolExecutionStartedEvent`, `ToolExecutionProgressEvent`,
`ToolExecutionFinishedEvent`, and `ToolResultEvent`
...
`TurnResult.termination` and `TurnFailed.termination` expose the same structured
diagnostic used by core generation and streaming. Aborted and failed turns also
attach it to the exception raised by `result()`.
...
`bytes`. `ToolOutput` image and file bytes use the same JSON-safe round trip.
Other tool result values must be JSON-compatible. Structured
termination diagnostics round-trip on both `turn.finished` and `turn.failed`.
