...
<ParamField path="on_reasoning_event" type="Callable[[ReasoningEvent], None]">
  Optional callback for provider reasoning output such as thinking deltas, summaries, signatures, or opaque state. These events are separate from `text_stream` and are not mixed into assistant text.
</ParamField>
...
<ParamField path="reasoning" type="ReasoningConfig">
  Normalized reasoning-depth controls like `effort` and `budget`.
</ParamField>

<Note>
For the conceptual model and provider-specific behavior of visible thinking output, see [`Thinking`](/explanation/thinking).
</Note>
...
```python
from ai_query import stream_text
from ai_query.providers import google, StepFinishEvent
