...
Start a remote turn using the server's `?stream=events` mode and reconstruct
each JSON payload into its public Python event dataclass.

```python
agent = connect("https://api.myapp.com/agent/researcher")

async for event in agent.events("Research prompt caching"):
    if event.type == "reasoning.delta":
        print(event.event.kind, event.event.text)
    elif event.type == "tool_call.delta":
        print(event.arguments_delta, end="")
    elif event.type == "turn.finished":
        print(event.result.usage)
```

`turn.failed` is yielded as a typed terminal event. HTTP failures, malformed
wire payloads, and `event: error` frames raise an exception from the iterator.
