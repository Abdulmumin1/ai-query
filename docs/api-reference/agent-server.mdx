...
<ParamField path="agent" type="Agent" required>
  The Agent instance to use as a template for each client. Each connected client gets their own isolated agent instance with the same configuration.
</ParamField>
...
```python
from ai_query import Agent, AgentServer, AgentServerConfig, openai, MemoryStorage
