...
<Note>
`stream_text` returns immediately with a `TextStreamResult` object. The actual streaming happens when you iterate over `text_stream`.
</Note>
...
| `stop_when` | `StopCondition` | Stop condition for tool loops |
