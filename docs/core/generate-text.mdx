...
| `messages` | `list[dict]` | Full conversation history (alternative to `prompt`) |
| `system` | `str` | System prompt to set context |
| `tools` | `dict[str, Tool]` | Tools the AI can call |
| `stop_when` | `StopCondition` | Stop condition for tool loops (e.g., `step_count_is(5)` or `has_tool_call("finish")`) |
