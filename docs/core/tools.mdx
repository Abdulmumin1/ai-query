...
Return `ToolOutput` when a tool needs to send rich content back to the model.
...
```python
from ai_query import FilePart, ImagePart, TextPart, ToolOutput, tool

@tool(description="Render the requested report")
def render_report() -> ToolOutput:
    return ToolOutput(content=[
        TextPart(text="The report and preview are attached."),
        ImagePart(image=preview_png, media_type="image/png"),
        FilePart(
            data=report_pdf,
...
Use ordinary strings, numbers, lists, or dictionaries for normal tool results.
A list is not treated as rich content unless it is wrapped in `ToolOutput`.

Rich tool outputs require upstream support:

- OpenAI uses the Responses API. Set `provider_options={"openai": {"api": "responses"}}`
  to use it from the first request; ai-query also selects it when rich output is
  present in a follow-up request.
...
- OpenAI-compatible chat-completions endpoints do not have a lossless rich
  tool-result representation.

Unsupported endpoint and model combinations raise
