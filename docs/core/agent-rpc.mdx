...
`AgentServer` automatically creates a REST endpoint for each action.
...
### 3. Agent-to-Agent (Internal RPC)

Use `self.call()` to get a type-safe proxy for communicating with another agent.
