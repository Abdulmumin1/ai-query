...
Set hooks on the agent as defaults:
...
class ChumpAgent(Agent[dict[str, Any]]):
    def __init__(self, id: str):
        super().__init__(
            ...
            hooks=AgentHooks(
                ...
                on_reasoning_event=self._on_reasoning_event,
            ),
        )

    async def _before_step(self, ctx):
        await self.emit("status", {"phase": "step_start", "step": ctx.step_number})
        return None

    async def _after_step(self, ctx):
        await self.emit("status", {"phase": "step_finish", "step": ctx.step_number})

    async def _before_tool_call(self, ctx):
        if ctx.event.tool_call.name == "bash":
            command = ctx.event.tool_call.arguments.get("command", "")
            if "rm -rf" in command:
                return BeforeToolCallResult(block=True, reason="Refusing destructive command.")
    ...
    async def _after_tool_call(self, ctx):
        return None

    async def _on_reasoning_event(self, event):
