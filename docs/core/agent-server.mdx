...
The Agent Server uses an **Agent Registry** to route requests.

1.  **Request**: `POST /agent/user-123/chat`
2.  **Registry**: Matches `user-123` to the registered `Agent` class.
3.  **Instantiation**: Creates (or retrieves) the agent instance for `user-123`.
4.  **Execution**: Runs the agent logic.
