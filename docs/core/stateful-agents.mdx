...
Agents can call other agents using `invoke()`. This is useful for splitting work across specialized agents.

### How It Works

1. **Agent A** calls `await self.invoke("agent-b", payload)`
2. **Agent B** receives the call in its `handle_invoke(payload)` method
...
```python
# Agent 1: Receives user messages, delegates research
...
# Agent 2: Handles research requests from other agents
class ResearcherAgent(ChatAgent, InMemoryAgent):
    async def handle_invoke(self, payload: dict) -> dict:
        """
        Receives: {"task": "search", "query": "..."}
        Returns:  {"answer": "..."}
        """
        if payload.get("task") == "search":
            answer = await self.chat(f"Research: {payload['query']}")
            return {"answer": answer}
        return {"error": "Unknown task"}
```

### Serverless with handle_request()

For AWS Lambda, Vercel, etc. (stateless environments):
...
| `invoke(agent_id, payload)` | Call another agent, wait for response |
| `emit(event, data)` | Fire event to subscribers |
| `handle_invoke(payload)` | Handle incoming invoke calls |
| `handle_request(request)` | Serverless lifecycle handler |
| `on_receive(message)` | Unified message handler |
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Communication" icon="messages" href="/cookbook/agent-communication">
    Build multi-agent systems with invoke, emit, and event patterns
  </Card>
  <Card title="Serverless Agents" icon="cloud" href="/cookbook/serverless-agents">
    Deploy to AWS Lambda, Vercel, and other serverless platforms
  </Card>
