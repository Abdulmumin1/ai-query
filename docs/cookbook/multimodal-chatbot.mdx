...
async def main():
    bot = PDFChatbot(google("gemini-2.5-pro"))
...
Images can be provided as URLs instead of base64:

```python
import asyncio
from ai_query import generate_text, google

async def analyze_url_image(url: str, question: str) -> str:
    result = await generate_text(
        model=google("gemini-2.5-flash"),
        messages=[
            {
...
async def main():
    response = await analyze_url_image(
