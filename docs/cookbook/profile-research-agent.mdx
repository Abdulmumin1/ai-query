...
from ai_query import Agent, AgentServer, AgentServerConfig, SQLiteStorage, tool, Field, step_count_is, google
...
1. **Simple API**: Just `await self.emit("event_type", data)` - no complex output adapters
